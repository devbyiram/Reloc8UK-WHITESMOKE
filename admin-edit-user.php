...
?>
		...
		<form method="post" action="admin-edit-user.php?id=<?php echo $user['id']; ?>" id="user-edit-form">
			...
			<div class="admin-crm-section" id="tenant-info-section">
				...
				<div class="row g-3">
					...
					<div class="col-md-6 col-sm-12">
						...
						<input id="date_of_birth" name="date_of_birth" class="form-control admin-crm-control" type="date" value="<?php echo date("Y-m-d", strtotime($user['date_of_birth'])); ?>">
					</div>
					<div class="col-md-6 col-sm-12">
...
$(document).ready(function() {
  ...
  var $userType = $('select[name="user_type"]');
  var $tenantInfo = $('#tenant-info-section');

  function toggleTenantInfo(clearOnHide) {
    if ($userType.val() == '3') {
      $tenantInfo.show();
    } else {
      $tenantInfo.hide();
      if (clearOnHide) {
        $tenantInfo.find('input, textarea').val('');
        $tenantInfo.find('select').prop('selectedIndex', 0);
      }
    }
  }

  toggleTenantInfo(false);
  $userType.on('change', function() {
    toggleTenantInfo(true);
  });
});
</script>
</body>
